...
def _build_messages(
    ...
    system = (
        ...
        "too — news that contradicts analyst consensus has more surprise. The "
        "1m/6m price change below is background for the thesis or risks, not a "
        "scoring input — a stock's recent run-up or sell-off does not tell you "
        "whether today's news is priced in, so do not discount a genuine "
        "catalyst (a real beat, an inflection on a de-rated name, etc.) just "
        "because the shares already moved beforehand. "
